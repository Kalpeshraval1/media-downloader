...
// ── In-memory cache (url+type -> formats data, 5 min TTL) ─
const CACHE = new Map();
function cacheGet(k) {
  const e = CACHE.get(k);
  if (!e) return null;
...
function cacheSet(k, d) {
  if (CACHE.size > 2000) { const keys = [...CACHE.keys()].slice(0,500); keys.forEach(x => CACHE.delete(x)); }
...
function buildFormats(info, mode) {
  ...
  // Best audio-only stream (for DASH pairing)
  ...
  // Muxed = video+audio in one file (no sync needed, works perfectly)
  const muxed = fmts
...
// ════════════════════════════════════════════════════════════
//  POST /api/download  — main info endpoint
// ════════════════════════════════════════════════════════════
app.post('/api/download', async (req, res) => {
  ...
  try {
    ...
    console.log('  yt-dlp error: ' + ytErr.message.slice(0, 120));
    if (isTikTok) {
      try {
    ...
    return res.status(500).json({ success: false,
      ...
      detail: ytErr.message.slice(0, 300),
      fix: 'Run: pip install -U yt-dlp' });
  }
});

// ════════════════════════════════════════════════════════════
//  GET /api/stream  — pipe media bytes with full browser headers
//  Supports Range requests (video seeking works)
//  Used for BOTH preview playback AND file download
// ════════════════════════════════════════════════════════════
app.get('/api/stream', (req, res) => {
  ...
  // Full browser-like headers so CDNs don't block
  const reqHeaders = {
  ...
  // Pass Range header through for video seeking
  ...
  const proxyReq = lib.request(opts, upstream => {
    ...
    // For download: set filename. For preview: inline.
    if (filename) {
    ...
    // Pass through size/range headers (required for video seeking + progress)
...
app.get('/health', (req, res) => {
  detectYtDlp(cmd => res.json({
    ok: true,
    ytDlp: cmd || 'NOT INSTALLED - run: pip install yt-dlp',
