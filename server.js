...
function cacheSet(k,d){if(CACHE.size>=MAX_CACHE){[...CACHE.keys()].slice(0,500).forEach(x=>CACHE.delete(x));}CACHE.set(k,{data:d,ts:Date.now()});}
...
function buildFormats(info, mode) {
  ...
  // Prefer muxed (has audio+video in one file) — no sync needed
  const muxed = fmts
    .filter(f => f.url && f.vcodec && f.vcodec !== 'none' && f.acodec && f.acodec !== 'none' && f.height)
    .sort((a,b) => (b.height||0) - (a.height||0));

  // DASH = video-only (YouTube 1080p+ is always DASH)
  const dash = fmts
...
app.post('/api/download', async (req, res) => {
  ...
  try {
    ...
    if (isTikTok) {
      try { const data=await tikWmFallback(url); const r={success:true,...data}; cacheSet(cKey,r); return res.json(r); }
...
// ════════════════════════════════════════════════════════════
//  GET /api/stream  — Proxy with full headers for browser playback
//  Sends correct Referer, User-Agent, Range support for seeking
// ════════════════════════════════════════════════════════════
app.get('/api/stream', (req, res) => {
  ...
  try {
    ...
    const proxy = lib.request(opts, upstream => {
      ...
      let ext = 'mp4';
      if (ct.includes('audio/mpeg') || ct.includes('audio/mp4')) ext = 'mp3';
      ...
      if (upstream.headers['content-length']) hdrs['Content-Length'] = upstream.headers['content-length'];
      if (upstream.headers['content-range'])  hdrs['Content-Range']  = upstream.headers['content-range'];
      if (upstream.headers['accept-ranges'])  hdrs['Accept-Ranges']  = 'bytes';

      res.writeHead(upstream.statusCode || 200, hdrs);
      upstream.pipe(res);
    ...
    proxy.end();
  } catch(e) { res.status(400).send('Invalid URL: ' + e.message); }
});
...
detectYtDlp(cmd => {
  if (cmd) console.log(`\nyt-dlp: ${cmd}`);
  else     console.warn(`\nWARN: yt-dlp not found! Run: pip install yt-dlp\n`);
