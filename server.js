...
function cacheSet(k, d) {
  if (CACHE.size > 2000) [...CACHE.keys()].slice(0, 500).forEach(x => CACHE.delete(x));
...
function ytDlpInfo(url) {
  return new Promise((resolve, reject) => {
    ...
    const proc = spawn(parts[0], args, { timeout: 60000 });
    let out = '', err = '';
    proc.stdout.on('data', d => out += d);
    proc.stderr.on('data', d => err += d);
    proc.on('close', code => {
      if (code !== 0) return reject(new Error(err.slice(0, 500) || 'yt-dlp exit ' + code));
      try { resolve(JSON.parse(out)); } catch (e) { reject(new Error('Bad JSON from yt-dlp')); }
    });
...
app.post('/api/download', async (req, res) => {
  ...
  try {
    ...
    const errMsg = ytErr.message;
    console.log('  yt-dlp error: ' + errMsg.slice(0, 200));
    ...
    // Return detailed error so frontend can show it
    return res.status(500).json({
      success: false,
      error: 'Could not fetch download links',
      detail: errMsg.slice(0, 400),
      platform,
      fix: errMsg.includes('Sign in') ? 'This video requires login - try a different URL'
         : errMsg.includes('Private') ? 'This video is private'
         : errMsg.includes('removed') ? 'This video has been removed'
...
app.get('/api/merge', (req, res) => {
  ...
  if (!FFMPEG) return res.status(503).send('ffmpeg not available on this server');
  ...
  // Stream ffmpeg output directly — no temp files, no disk usage
  const ff = spawn('ffmpeg', [
    '-hide_banner', '-loglevel', 'error',
    '-i', videoUrl,
    '-i', audioUrl,
    '-c:v', 'copy',        // copy video as-is (fast, no quality loss)
  ...
  ff.stderr.on('data', d => {
    ...
    if (!line.includes('frame=') && !line.includes('size=') && line.length > 0)
      console.log('[ffmpeg] ' + line.slice(0, 100));
  });

  ff.on('close', code => {
    if (code !== 0) {
      console.log('[ffmpeg] exit ' + code + ': ' + ffErr.slice(-200));
      if (!res.headersSent) res.status(500).send('ffmpeg error: ' + ffErr.slice(-200));
    }
  });

  ff.on('error', err => {
    console.log('[ffmpeg spawn error] ' + err.message);
    if (!res.headersSent) res.status(500).send('ffmpeg spawn error: ' + err.message);
  });
...
// ════════════════════════════════════════════════════════
//  GET /api/stream  -- proxy single stream
// ════════════════════════════════════════════════════════
